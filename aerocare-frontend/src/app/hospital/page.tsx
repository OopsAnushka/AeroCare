...
import {
  ...
  AlertCircle, CheckCircle2, Activity, Ambulance, BedDouble, Stethoscope,
} from 'lucide-react';
...
import { createUserProfile, saveUserProfile } from '@/lib/firestore';
...
const FEATURES = [
  ...
  { icon: Activity,    label: 'Resource Sync',       desc: 'Push ventilator & OT availability to the AeroCare network instantly.' },
...
export default function HospitalAuthPage() {
  ...
  // Redirect authenticated hospital users to dashboard (after auth loads)
  useEffect(() => {
  ...
  useEffect(() => {
    setCities(selectedState ? (CITY_MAP[selectedState] ?? []) : []);
  }, [selectedState]);
  ...
  return (
    <div className="min-h-screen flex bg-[#06090f] text-white font-sans overflow-hidden">
      ...
      <div className="hidden lg:flex w-[44%] flex-col relative overflow-hidden">
        {/* Gradient blobs */}
        <div className="absolute inset-0 bg-gradient-to-br from-emerald-950 via-[#06090f] to-teal-950" />
        <div className="absolute top-[-10%] left-[-10%] w-[70%] h-[70%] bg-emerald-500/10 rounded-full blur-[100px]" />
        ...
        {/* Back link */}
        <Link href="/" className="relative z-10 flex items-center gap-2 text-emerald-400/70 hover:text-emerald-300 transition-colors font-bold text-sm m-8 w-fit">
        ...
        <div className="relative z-10 flex flex-col justify-center flex-1 px-12 pb-16">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-2 rounded-full text-xs font-bold mb-8 w-fit backdrop-blur-sm">
          ...
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            className="text-gray-400 text-base font-medium leading-relaxed mb-12 max-w-sm">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}
            className="flex gap-8 mt-12 pt-10 border-t border-white/5">
      ...
      {/* ══ RIGHT FORM PANEL ══ */}
      <div className="flex-1 flex items-start lg:items-center justify-center p-6 sm:p-10 bg-[#0a0f18] overflow-y-auto">
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
          className="w-full max-w-[460px] my-8 lg:my-0">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={mode} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
              <h2 className="text-2xl font-extrabold text-white mb-1">
                {mode === 'login' ? 'Welcome back' : 'Register your facility'}
              </h2>
              <p className="text-sm text-gray-500 mb-6">
                {mode === 'login' ? 'Access your hospital command dashboard.' : 'Join the AeroCare emergency response network.'}
              </p>
            </motion.div>
          </AnimatePresence>
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-3">
              ...
              <motion.div key={mode} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }}>
                ...
                {mode === 'register' && (
                  <div className="space-y-3 mb-3">
                    {/* Hospital Name */}
                    <HField id="hospitalName" label="Hospital / Facility Name" icon={Building2} placeholder="e.g. Bombay Hospital & Medical Research Centre" required />
                    ...
                    {/* State & City */}
                    <div className="grid grid-cols-2 gap-3">
                      <HSelect id="state" label="State" icon={MapPin} value={selectedState}
                        onChange={(v) => setSelectedState(v)} options={INDIAN_STATES} placeholder="Select State" required />
                      <HSelect id="city" label="City" icon={MapPin} value="" onChange={() => {}}
                        options={cities} placeholder={selectedState ? 'Select City' : 'Select State first'} required />
                    </div>
                    ...
                    {/* Contact Person */}
                    <div className="grid grid-cols-2 gap-3">
                      <HField id="contactPerson" label="Admin / Contact Person" icon={ShieldCheck} placeholder="Full Name" />
                ...
                <div>
                  ...
                  <div className="relative flex items-center rounded-xl border border-white/10 bg-white/5 transition-all focus-within:border-emerald-500/50 focus-within:bg-white/[0.07] focus-within:shadow-[0_0_0_3px_rgba(16,185,129,0.1)]">
                    ...
                    <button type="button" onClick={() => setShowPw(!showPw)}
                      className="absolute right-3 text-gray-600 hover:text-gray-400 transition-colors">
            ...
            {mode === 'login' && (
              <div className="text-right">
                <button type="button" onClick={handleForgotPassword}
                  className="text-xs text-emerald-400/70 hover:text-emerald-400 font-bold transition-colors">
            ...
            <motion.button whileHover={{ scale: 1.015 }} whileTap={{ scale: 0.985 }} type="submit"
              ...
              {loading ? (
                ...
                <>
                  {mode === 'login' ? 'Access Dashboard' : 'Register Facility'}
...
function HSelect({ id, label, icon: Icon, options, placeholder, required = false, value, onChange }: {
  id: string; label: string; icon: any; options: string[]; placeholder: string; required?: boolean;
  ...
  return (
    <div>
      ...
      <div className="relative flex items-center rounded-xl border border-white/10 bg-white/5 transition-all focus-within:border-emerald-500/50 focus-within:bg-white/[0.07] focus-within:shadow-[0_0_0_3px_rgba(16,185,129,0.1)]">
        ...
        <select name={id} id={id} required={required} defaultValue=""
